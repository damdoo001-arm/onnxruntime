...
def collect() -> tuple[list[dict[str, object]], dict[str, str]]:
    ...
    metadata = {
        "kai_release": kai_release,
        "ort_release": ort_release,
        "ort_pin_label": ort_pin.label,
        "ort_pin_revision": ort_pin.revision,
...
<meta name="description" content="Arm® KleidiAI™ software compatibility in ONNX Runtime"><title>Arm® KleidiAI™ software compatibility in ONNX Runtime</title><style>__BASE_CSS__
...
<div class="kleidiai-page">
  <h1>Arm® KleidiAI™ software compatibility in ONNX Runtime</h1>
  <p class="kai-lede">This page outlines support details for the <a href="https://www.arm.com/markets/artificial-intelligence/software/kleidi">Arm KleidiAI software</a> micro-kernel library. Micro-kernels are integrated into ONNX Runtime via Microsoft Linear Algebra Sub-Process (MLAS) to support CPU-Based inference acceleration on Arm-Based CPUs.</p><p class="muted trademark-notice">Arm, Kleidi, KleidiAI, KleidiCV and Kleidi Libraries are registered trademarks or trademarks of Arm Limited (or its subsidiaries or affiliates) in the US and/or elsewhere.</p>
  <p class="kai-meta"><span>KleidiAI tagged release: <a href="__KAI_URL__"><code>__KAI_RELEASE__</code></a></span><span>ONNX Runtime release: <a href="__ORT_URL__"><code>__ORT_RELEASE__</code></a></span><span>ONNX Runtime KleidiAI pin: <a href="__KAI_PIN_URL__"><code>__KAI_PIN__</code></a></span><span>Last updated date: <time datetime="__UPDATED_AT__">__UPDATED_AT__</time></span></p>
...
def main() -> None:
    ...
    output = (
        ...
        .replace("__KAI_URL__", f"{report.KAI_GITHUB}/tree/{metadata['kai_release']}")
        .replace("__KAI_RELEASE__", metadata["kai_release"])
        .replace("__ORT_URL__", f"{report.ORT_GITHUB}/tree/{metadata['ort_release']}")
        .replace("__ORT_RELEASE__", metadata["ort_release"])
        .replace("__KAI_PIN_URL__", f"{report.KAI_GITHUB}/tree/{metadata['ort_pin_revision']}")
        .replace("__KAI_PIN__", metadata["ort_pin_label"])
        .replace("__UPDATED_AT__", metadata["updated_at"])
    )
    ...
    print({"microkernels": len(records), "kleidiai": metadata["kai_release"]})
